...
#引用包
library(limma)
inputFile="tcga.INFexp.txt"    #输入文件
fdrFilter=0.05                 #fdr临界值
logFCfilter=1                  #logFC临界值
setwd("D:\\biowolf\\inflammatory\\10.tcgaDiff")   #设置工作目录
...
#输出所有基因的差异情况
write.table(outTab,file="tcga.all.xls",sep="\t",row.names=F,quote=F)

#输出差异表格
outDiff=outTab[( abs(as.numeric(as.vector(outTab$logFC)))>logFCfilter & as.numeric(as.vector(outTab$fdr))<fdrFilter),]
write.table(outDiff,file="tcga.diff.xls",sep="\t",row.names=F,quote=F)
...
write.table(heatmap,file="tcga.diffExp.txt",sep="\t",col.names=F,quote=F)
